...
```
---
#Code PlantULM ERD```bashbash
@startuml
entity "NHAN_VIEN" as NhanVien {
  *ID : int <<PK>>
  --
  MaNV : string
  ChucVu : string
...
entity "KHACH_HANG" as KhachHang {
  *ID : int <<PK>>
  --
  MaKhachHang : string
  TenKhachHang : string
...
entity "THANH_TOAN" as ThanhToan {
  *ID : int <<PK>>
  --
  MaThanhToan : string
  MaLich : int <<FK>>
